...
export async function apiRequest<T>(path: string, init: RequestInit = {}, token?: string | null): Promise<T> {
  try {
    ...
    if (!response.ok) {
      ...
      throw new ApiError(response.status, response.status === 403 ? 'You do not have permission to perform this action.' : body.message || 'The request could not be completed.');
    }
