...
import { ContractActivationDialog } from '../components/ContractActivationDialog';
import type { ContractDetails } from '../types/contracts';
import { formatMoney } from '../utils/format';
import { canActivateContracts } from '../utils/roles';

export function ContractDetailPage() {
  const { id } = useParams(); const location = useLocation(); const { token, user } = useAuth(); const routeNotice = (location.state as { notice?: string } | null)?.notice;
  const [details, setDetails] = useState<ContractDetails>(); const [loading, setLoading] = useState(true); const [error, setError] = useState<{ status: number; message: string }>(); const [activating, setActivating] = useState(false); const [notice, setNotice] = useState(routeNotice || ''); const [conflict, setConflict] = useState('');
  const load = async () => { if (!token) return undefined; const value = await getContract(Number(id), token); setDetails(value); return value; };
  useEffect(() => { setLoading(true); load().catch(reason => setError({ status: reason instanceof ApiError ? reason.status : 0, message: reason instanceof ApiError ? reason.message : 'Contract details could not be loaded.' })).finally(() => setLoading(false)); }, [id, token]);
  ...
  const mayActivate = contract.status === 'Draft' && !!user && canActivateContracts(user.role);
  const activated = async () => { await load(); setActivating(false); setConflict(''); setNotice('Contract activated successfully.'); };
  const conflicted = async (message: string) => { setConflict(message); const current = await load().catch(() => undefined); if (current?.contract.status !== 'Draft') setActivating(false); };
  return <section>{notice && <div className="success-notice" role="status">{notice}</div>}{conflict && <div className="error activation-page-error" role="alert">{conflict}</div>}<div className="page-title"><div><p className="eyebrow">Contract #{contract.contractId}</p><h1>{contract.contractNumber}</h1><p>{contract.customerName}</p></div><div className="page-actions">{mayActivate && !activating && <button className="primary" onClick={() => { setConflict(''); setActivating(true); }}>Activate contract</button>}<Link className="button-link secondary" to="/contracts">Back to contracts</Link></div></div>
    ...
    {activating && mayActivate && <ContractActivationDialog contract={contract} token={token!} onClose={() => setActivating(false)} onActivated={activated} onConflict={conflicted} />}
  </section>;
}
