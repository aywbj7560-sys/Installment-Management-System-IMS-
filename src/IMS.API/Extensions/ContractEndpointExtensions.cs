...
public static class ContractEndpointExtensions
{
    public static void MapContractEndpoints(this WebApplication app)
    {
        ...
        group.MapPost("/{id:long}/activate",async(long id,ActivateContractRequest request,ClaimsPrincipal user,IContractService service,CancellationToken ct)=>
        {
            var errors=request.Validate();
            if(errors.Count>0) return Results.ValidationProblem(errors);
            if(!long.TryParse(user.FindFirstValue("sub"),out var userId)) return Results.Unauthorized();
            var result=await service.ActivateAsync(id,request,userId,ct);
            return result.ErrorStatus==0 ? Results.Ok(result.Contract) : Results.Problem(statusCode:result.ErrorStatus,detail:result.Message);
        }).RequireAuthorization(p=>p.RequireRole(RoleNames.Admin,RoleNames.FinancialManager));
